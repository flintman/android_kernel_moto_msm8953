...
$(TARGET_PREBUILT_INT_KERNEL): $(KERNEL_OUT) $(KERNEL_CONFIG) $(KERNEL_HEADERS_INSTALL)
	...
	$(MAKE) -C $(TARGET_KERNEL_SOURCE) KBUILD_RELSRC=$(KERNEL_SOURCE_RELATIVE_PATH) O=$(BUILD_ROOT_LOC)/$(KERNEL_OUT) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) $(KERNEL_CFLAGS)
	$(MAKE) -C $(TARGET_KERNEL_SOURCE) KBUILD_RELSRC=$(KERNEL_SOURCE_RELATIVE_PATH) O=$(BUILD_ROOT_LOC)/$(KERNEL_OUT) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) $(KERNEL_CFLAGS) dtbs
	$(MAKE) -C $(TARGET_KERNEL_SOURCE) KBUILD_RELSRC=$(KERNEL_SOURCE_RELATIVE_PATH) O=$(BUILD_ROOT_LOC)/$(KERNEL_OUT) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) $(KERNEL_CFLAGS) modules
	$(MAKE) -C $(TARGET_KERNEL_SOURCE) KBUILD_RELSRC=$(KERNEL_SOURCE_RELATIVE_PATH) O=$(BUILD_ROOT_LOC)/$(KERNEL_OUT) INSTALL_MOD_PATH=$(BUILD_ROOT_LOC)../$(KERNEL_MODULES_INSTALL) INSTALL_MOD_STRIP="--strip-debug --remove-section=.note.gnu.build-id" ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) modules_install
	$(mv-modules)
	$(clean-module-folder)
